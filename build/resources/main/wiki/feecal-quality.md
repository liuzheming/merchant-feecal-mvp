...
### 发布与配置管理
- **灰度开关**：在 `commerce-merchant-config-start` 的配置中预置灰度比例、止损开关，支持快速退回旧逻辑。
- **双轨发布**：资金敏感模块须走“暗发布 + 对账核验 + 开关切换”流程，并在灰度期间隔离风险商户。
- **配置审核**：所有资金相关配置变更都走双人审核，并在 `wiki` 记录变更单；自动对比新旧配置差异。
...
## 面试官常问质量/资金安全话题（现状 + 改进）
> 下面的口径按“面试现场”来写：先说明 MVP 已落地的保护，再补充计划中的增强，让面试官看到我们既懂架构也有质量兜底思路。
